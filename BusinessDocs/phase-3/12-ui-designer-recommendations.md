...
Brand tokens are placeholders and late value changes can create broad visual
rework.
...
1. Publish token schema v0.1 in `docs/brand/design-tokens.json` with
   `PLACEHOLDER:` values only.
2. Define migration-safe aliases (e.g., `color-surface-primary` -> never renamed
   after lock).
3. Require Storybook components to consume only semantic aliases, not raw color
   values.
...
Late brand decisions could trigger pervasive CSS/component refactors and
inconsistent visuals across screens.
...
Component inventory file is absent, risking naming drift and duplicate
components.
...
1. Create `docs/storybook/component-inventory.md` with canonical names
   and ownership.
...
Motion behavior is not standardized and can produce inconsistent or heavy
transitions.
...
2. Assign allowed motion patterns per component class (modal, toast, accordion,
   tab switch).
...
Accessibility validation of color/focus is pending and can block release
quality.
...
2. Validate contrast for text, badges, controls, and focus rings against WCAG
   AA.
...
Potential WCAG non-compliance, blocked releases, and reduced usability for
...
Theme switching behavior is under-specified for persistence and fallback
handling.
...
1. Define precedence order: explicit user setting > system preference > product
   default.
