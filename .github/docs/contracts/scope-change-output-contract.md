...
> Version: 1.0 | Defines the mandatory output structure for the Scope Change
> Agent (Agent 37)
...
Ensures that fundamental project premise changes (business model pivots, core
architecture changes, target audience shifts) are processed through a structured
scope change protocol. The Scope Change Agent produces a delta report, places
affected tickets on backlog hold, and prepares Sprint Gate reconciliation data
to update the Master Synthesis.

---

## OUTPUT FILE

**Location:** `.github/docs/synthesis/scope-change-[N].md` **Format:** Markdown
...
- Sprint IDs affected
- Recommended Sprint Gate actions: `HOLD` | `PROCEED_WITH_DELTA` |
  `RESTART_SPRINT`
...
- [ ] Every affected ticket has a status transition (`SCOPE_CHANGE_HOLD SC-[N]`,
...
> No standalone JSON export for this contract. Scope change output is
> Markdown-only; affected ticket lists and reconciliation data are consumed from
> the structured Markdown sections.
