...
> Version: 1.0 | Defines the mandatory output structure for the Documentation
> Agent (Agent 26)
...
Ensures project documentation is kept current after every sprint. The
Documentation Agent updates the user manual, technical manual, and changelog,
reporting exactly what changed and flagging any inconsistencies or missing
documentation that could impact the next sprint or end-user experience.
...
- `DOC_INCONSISTENCY:` — Detected inconsistency between code and documentation
  (with details)
- `DOC_PENDING:` — Documentation that cannot be completed yet (with reason)
- `DOC_MISSING:` — Required documentation that does not exist yet (with
  specification)
...
- [ ] User manual and technical manual sections are explicitly reported (even if
      "no changes")
- [ ] CHANGELOG.md is updated with the sprint's changes
- [ ] All `DOC_INCONSISTENCY` signals include the specific inconsistency and
      file references
...
> No standalone JSON export for this contract. Documentation output is
> Markdown-only.
