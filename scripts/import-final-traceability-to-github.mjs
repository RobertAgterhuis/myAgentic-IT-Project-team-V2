...
function ensureMilestone(repo, title) {
  const milestones = ghApi(`repos/${repo}/milestones?state=all&per_page=100`, { method: "GET" }) || [];
...
function createIssue(repo, payload) {
  const fields = [["title", payload.title], ["body", payload.body]];
...
function main() {
  ...
  const existingIssues = ghApi(`repos/${REPO}/issues?state=all&per_page=100`, { method: "GET" }) || [];
