...
> mechanisms. For a user-facing guide, see `src/webapp/ui/src/help/decisions.md`.
...
1. **Open Questions table** — uncategorized items (type `DECIDED` or
   `OPEN_QUESTION`, status `OPEN`/`DECIDED`/`DEFERRED`/`EXPIRED`).
2. **Category Registry table** — a lookup table mapping each technology category
   to its file, decision count, status (`ACTIVE`/`PARTIAL`/`DEFERRED`), and
   applicability flag.
...
`PARTIAL` files additionally contain a "Deferred Decisions" subsection for
individually deferred rows. `DEFERRED` files include a `> Deferred-Reason:` line
explaining why.

**Body:** A markdown table with columns
`ID | Priority | Scope | Decision | Notes | Date`. Each row is one `DECIDED`
item.
...
3. **Filter OPEN HIGH:** Items where `priority = HIGH` and `scope` matches the
   current sprint or its stories → **block the Sprint Gate**. Present to the
   user with instructions to answer.
4. **Filter OPEN MEDIUM/LOW:** List as informational, non-blocking.
5. **Store constraints:** Aggregate all `DECIDED` items (from index + active
   categories) into a sprint-scoped constraint set for injection in step 5 of
   sprint start.
...
2. Skip DEFERRED category files. Skip individually deferred rows within PARTIAL
   files.
3. Filter by scope → keep only decisions relevant to the current sprint, its
   stories, or the active agents.
4. Inject the filtered set as **hard constraints** into each agent's context
   block.
5. Log which decisions (with category source) were injected in the Orchestrator
   Log.
...
- `DEFERRED_TECH_REQUIRED` — Implementation Agent needs the technology before
  coding.
- `DEFERRED_TECH_DETECTED` — Test Agent or PR/Review Agent finds deferred tech
  in code/diff.
...
**Exception path:** User types `SKIP ACTIVATION [category]` → Orchestrator
writes a `DECIDED` exception item to the index file and reverts the category to
`DEFERRED` if already activated.

---

## 4. Enforcement chain (Phase 5 triple-check)

Three agents independently validate decision compliance. A violation at any
point blocks progress.
...
3. If the story requires a technology matching a DEFERRED category → **HALT**.
   Report `DEFERRED_TECH_REQUIRED: [category]`. Wait for Orchestrator
   auto-activation (ORC-45), then resume with newly active constraints.
...
2. Filter by scope overlap with the current story's domain (file types,
   framework).
3. Verify each applicable `DECIDED` item against the implementation code.
4. On violation: `DEC-VIOLATION: [DEC-ID] — [expected vs actual]`. Return to
   Implementation Agent.
5. Deferred technology detection: if code introduces files matching a DEFERRED
   category → `DEFERRED_TECH_DETECTED: [category]`. Escalate to Orchestrator.
6. Document in Test Report:
   `DECISION COMPLIANCE: Applicable: [N], Compliant: [N], Violations: [list or NONE]`.
...
On match with a DEFERRED category → block merge → escalate to Orchestrator for
auto-activation.
...
The web UI (`npm start`, port 3000) reads and writes the
same markdown files:
...
The Orchestrator reads the same files — changes made via the web UI are picked
up at the next Sprint Gate automatically.
...
- `category_activated: [file]` — set by ORC-45 when a deferred category is
  auto-activated mid-sprint.
- Sprint constraints are not persisted in session state; they are recomputed at
  each Sprint Gate from the decision files.
