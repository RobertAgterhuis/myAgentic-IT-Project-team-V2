...
**Audit Trail:** `docs/decisions.md`,
`docs/session/session-state.json`, mutation log at
`docs/session/mutation-log.jsonl`
...
`docs/decisions.md` and Sprint Gate reviews.
...
**Memory Protocol:** See `docs/guardrails/00-global-guardrails.md`
...
- **Decision compliance:** Code violates `DECIDED` constraint in
  `docs/decisions.md` → Revert or decision amendment
...
**Protocol:** See `docs/phase-5/sprint-1-standup-protocol.md`
...
**Owner:** KPI Agent + PM  
**Tracked Metrics:** See `docs/phase-5/sprint-1-kpi-log.md`
...
- **Decisions & Open Questions:** `docs/decisions.md` (human-editable) +
  `docs/decisions/*.md` (category files)
- **Session State:** `docs/session/session-state.json` (phase status,
  blocker tracking, GitHub integration metadata)
- **Mutation Log:** `docs/session/mutation-log.jsonl` (append-only audit
  trail of all questionnaire/decision changes)
- **Sprint Reports:** `docs/phase-5/sprint-[N]-completion-report.md`
  (per sprint)
- **Retrospectives:**
  `docs/retrospectives/sprint-[N]-retrospective.md` +
  `lessons-learned.md` + `velocity-log.json`
- **GitHub Sync:** `docs/github/sync-report-[TIMESTAMP].md` (execution
...
**Proposal format:** Create entry in `docs/decisions.md` with
...
- [ ] All decisions documented in `docs/decisions.md` with status
...
- [x] Guardrails from /docs/guardrails/ have been checked → G-GLOB-50
