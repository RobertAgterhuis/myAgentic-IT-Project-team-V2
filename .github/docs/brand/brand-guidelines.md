...
Version: 2.0  
Date: 2026-03-10  
Source: `.github/docs/phase-4/14-brand-strategist-analysis.md` and `.github/docs/phase-4/14-brand-strategist-recommendations.md`

## 1. Brand Overview

Mission: Enable professional teams to design and deliver complete software solutions through a structured, multi-discipline, evidence-driven process.

Vision: A world where software creation combines strategic rigor with execution excellence, eliminating the gap between planning and delivery.
...
For product managers, software architects, and technical leads in mid-market and enterprise organizations, Agentic SDLC is an end-to-end solution design and delivery system that combines strategic analysis (Phases 1-4) with iterative implementation (Phase 5), because teams need both planning rigor and execution speed.
...
Clear space:
- Minimum clear space equals the height of uppercase `A` in `Agentic` on all sides.
...
Token source: `.github/docs/brand/design-tokens.json` (`colors` object).
...
- Always pair semantic colors (`error`, `warning`, `success`) with icons or labels.
- Do not introduce raw hex values in UI implementation.

## 4. Typography

Token source: `.github/docs/brand/design-tokens.json` (`typography` object).
...
- Use `normal` line-height for UI text and `relaxed` for long-form guidance content.
...
- Show real process artifacts (phase maps, risk matrices, sprint traces) over stock photography.
- Maintain neutral, professional composition with high information density and clear whitespace.
...
- Prefer visual metaphors that reflect orchestration, sequencing, and traceability.
...
- Use consistent canonical terms: phase, sprint, risk, blocker, handoff, mitigation.
...
- Product, documentation, and campaign copy must align with this voice standard before publication.
