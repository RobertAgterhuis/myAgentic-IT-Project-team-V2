...
| DEC-271 | HIGH     | Phase 4, Phase 5 (Analytics)         | Matomo cookieless replaces Google Tag Manager as the analytics platform for GDPR compliance                 | Matomo runs self-hosted (docker-compose.analytics.yml), cookieless mode (`disableCookies`), no consent banner required. CORS via matomo-nginx.conf. GTM is not used — "GTM" in sprint stories refers to Go-To-Market messaging, not Google Tag Manager. | 2026-05-10 |
