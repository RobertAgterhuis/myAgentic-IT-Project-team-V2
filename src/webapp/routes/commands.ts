...
export = function createCommandRoutes(ctx): Record<string, unknown> {
  ...
  function buildCommandEntry(body: Record<string, unknown>): Record<string, unknown> {
    return {
      command: (body.command as string).trim(),
      project: ((body.project || '') as string).trim() || null,
      description: ((body.description || '') as string).trim() || null,
      scope: ((body.scope || '') as string).trim() || null,
  ...
  async function apiPostCommand(req, res) {
    ...
    const cmd = (body.command as string).trim().toUpperCase();
    ...
    const cmdResponse = {
      ...
      message: R.commandQueued(entry.clipboard_text as string),
    };
