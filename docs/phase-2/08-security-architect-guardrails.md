...
Must not bind service to non-loopback interfaces in deployed environments unless
authentication mode is enabled and validated.
...
Every non-public endpoint must declare required role/permission and include
deny-path tests.
...
CI security gates must include secret scan, SAST, SCA, DAST, and container scan
with approved fail thresholds.
...
Write endpoints must enforce rate limiting and return `429` when thresholds are
exceeded.
...
- Applies to: source code, request handling, logging pipeline, persisted
  documents
- Time horizon: permanent

### Rule

Hardcoded secrets are prohibited; runtime secret pattern detection and CI secret
scans must remain enabled.
...
- Time horizon: until Data Architect + Legal Counsel outputs are complete, then
  permanent enforcement

### Rule

Retention, deletion, and redaction controls must not be marked complete before
approved data classification matrix exists.
