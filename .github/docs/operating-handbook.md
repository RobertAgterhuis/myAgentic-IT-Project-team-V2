...
```bash
node .github/webapp/server.js 2>&1 | tee server.log
```
...
```bash
cat .github/docs/session/session-state.json | node -e "process.stdin.on('data',d=>console.log(JSON.parse(d).status))"
```
...
PORT=3001 node .github/webapp/server.js
```
...
1. Check for `.bak` files in `.github/docs/session/`
...
If `contrast.test.js` fails, verify that `.github/docs/brand/design-tokens.json`
...
- **File**: Read `.github/docs/session/session-state.json`
