...
nav_order: 5
description:
  Visual identity, color palette, typography, spacing tokens, and component
  design principles.
---
...
**Mission:** Enable AI Project Leads to orchestrate multi-agent software
creation and audit workflows through a clear, accessible, and trustworthy web
interface.
...
**Design Philosophy:** Zero-dependency, single-file architecture. Every visual
decision must work within inline CSS/JS constraints. Simplicity over decoration.

---

## 2. Logo Usage

**Current State:** The application uses a text-based header with an emoji logo
mark.
...
- Do not display the logo on backgrounds that fail WCAG AA contrast against
  `header-text`
...
All colors are defined in `design-tokens.json` under the `color` category. Both
...
- **Interactive elements:** Focus ring (`primary`) on `bg`/`surface` must meet
  ≥3:1 non-text contrast
- **Status colors on `surface`:** `success`, `warning`, `danger` must be paired
  with icons/text — never color alone (colorblind safety)

**Colorblind Safety Rule:** All status indicators must use color + icon + text.
Decision badges use Unicode symbols alongside color: ✓ (decided), ○ (open), ◇
(deferred).

### Forbidden Combinations

- Never use `danger` as a background color for large areas
- Never use `warning` text on light `bg` without bold weight (insufficient
  contrast risk)
...
All typography tokens are defined in `design-tokens.json` under the `typography`
category.
...
- Always use the `sans` family for UI; `mono` only for code, IDs, and command
  syntax
...
The application uses an inline SVG icon system defined in JavaScript
(`iconFor()` function in `index.html`).
...
- Icon size follows the context: 16px in badges, 18px in sidebar, 24px in
  headers
- Icons use `currentColor` for fill — they inherit the text color from their
  container
...
Not applicable — this is a developer tool with no photographic or illustrative
content. All visual communication is through icons, color-coded status, and data
visualization (pipeline, progress bars).
...
Example: "Unable to save. The server may be temporarily unavailable — try again
in a moment."
...
- "Questionnaire" (not form/survey), "Decision" (not issue/ticket), "Agent" (not
  bot/assistant)
