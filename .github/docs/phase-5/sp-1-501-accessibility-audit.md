...
- No modifications to `design-tokens.json` are permitted after this date
  without **UX Lead + PM co-approval** via PR review
...
**Design Token Support:** `colors.focusRing` (#2F80ED) provides visible
2px outline on all interactive elements. Focus indicator contrast ratio 4.1:1
...
- Badge component specification: "Never convey meaning by color alone. Pair
  with text or icon label."
...
**Rationale:** Sprint 1 does not ship customer-facing complex data tables.
The command center web app uses simple tables for questionnaire and decision
