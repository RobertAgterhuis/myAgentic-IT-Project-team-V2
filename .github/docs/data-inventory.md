...
| **Issue** | #139 |
| **Depends on** | `.github/docs/ga-definition.md` (deployment profile), `.github/docs/security-design.md` (data-at-rest) |

---

## 1. Data Categories

All data stored by the Agentic SDLC Platform in the v1 GA (localhost/single-operator) configuration.

### 1.1 Session Data

| Data Element | Format | Location | Contains PII | Retention |
|-------------|--------|----------|-------------|-----------|
| Session state | JSON | `.github/docs/session/session-state.json` | No | Project lifetime |
| Session audit trail | JSON | `.github/docs/session/session-state-audit.json` | No | Project lifetime |
| Command queue | JSON | `.github/docs/session/command-queue.json` | No | Overwritten per command cycle |
| Reevaluate trigger | JSON | `.github/docs/session/reevaluate-trigger.json` | No | Transient (consumed on read) |
...
| Decisions log | Markdown | `.github/docs/decisions.md` | No | Project lifetime |
| Decision records | Markdown | `.github/docs/decisions/` | No | Project lifetime |

### 1.3 Phase Deliverables

| Data Element | Format | Location | Contains PII | Retention |
|-------------|--------|----------|-------------|-----------|
| Phase 1–4 analyses | Markdown | `.github/docs/phase-{1,2,3,4}/` | No | Project lifetime |
| Synthesis reports | Markdown | `.github/docs/synthesis/` | No | Project lifetime |
| Sprint plans/reports | Markdown | `.github/docs/phase-5/` | No | Project lifetime |
| Brand assets | JSON, Markdown | `.github/docs/brand/` | No | Project lifetime |
| Storybook inventory | Markdown | `.github/docs/storybook/` | No | Project lifetime |
...
For v1 GA (localhost/single-operator), the operator IS the data subject. DSAR procedures are documented for completeness and for future multi-user profiles.

### 3.2 Data Export

To export all data stored by the platform:

1. **Project data:** Copy the entire repository directory (includes all Markdown, JSON, and configuration files)
2. **Session state:** The file `.github/docs/session/session-state.json` contains the complete session state
3. **Questionnaire answers:** All files under `BusinessDocs/` contain business input data
4. **Analytics data:** Export Matomo data via Matomo's built-in GDPR export tool (Admin → Privacy → Export)
...
2. **Session data only:** Delete session files while preserving deliverables
   ...
   ```bash
   rm .github/docs/session/session-state.json
   rm .github/docs/session/session-state-audit.json
   rm .github/docs/session/command-queue.json
   ```
...
4. **Git author data:** Use `git filter-branch` or BFG Repo-Cleaner to rewrite history (destructive operation)

### 3.4 Right to Rectification

All data is stored in human-readable formats (Markdown, JSON). The operator can directly edit any file to correct inaccurate data.
...
| **Technical/organizational measures** | See `.github/docs/security-design.md` |
...
When the platform moves to Profile 2 (Internal Team) or Profile 3 (Internet-Exposed):
...
Operator (local browser)
    ...
    │                        │                    ├── .github/docs/ (deliverables, session state)
...
**Key property:** No data leaves the local machine in the v1 GA configuration. All persistence is file-backed within the project directory or Docker volumes on the same host.
...
*This document catalogs all data processed by the Agentic SDLC Platform and defines retention, export, and deletion procedures. For v1 GA (localhost/single-operator), no personal data leaves the local machine.*
