...
`.github/docs/storybook/component-inventory.md`. Stories for new UI components
...
still produces `.github/docs/brand/design-tokens.json`. The Storybook Agent is
never skipped.
...
in `.github/docs/decisions.md` as a new `DECIDED` item. This applies regardless
...
2. Activate Questionnaire Agent (generation workflow) — produces questionnaire
   ...
   `.github/docs/contracts/questionnaire-output-contract.md`
3. Activate Questionnaire Agent (document generation workflow) — updates the 2
   ...
   validate against `.github/docs/contracts/questionnaire-output-contract.md`
...
`LESSON_CANDIDATE` in `.github/docs/retrospectives/lessons-learned.md`,
...
- Recorded in `.github/docs/retrospectives/velocity-log.json` as a separate
  entry with `"type": "HOTFIX"`
- A `DECIDED` item in `.github/docs/decisions.md` if the hotfix implies a
...
- Overwrites only the scan fields in
  `.github/docs/onboarding/onboarding-output.md`; intake answers are preserved
...
3. On `COMPLETE` or `PARTIAL`: verify `.github/docs/brand/design-tokens.json` is
   present and valid JSON
4. Verify: `.github/docs/brand/brand-assets-report.md` present? 4b. Verify:
   `.github/docs/brand/brand-guidelines.md` present with sections 1–6? Missing
...
`.github/docs/session/session-state.json` exists with `status ≠ COMPLETE`. If
yes: present the resumable session to the user per
`.github/docs/contracts/session-state-contract.md` and wait for choice RESUME or
RESET.
...
4. **Decisions via web UI:** The user may create new decisions (`DECIDED` or
   ...
   to category files under `.github/docs/decisions/` (decided items organized by
...
1. At every session start and before every Sprint Gate, check whether
   `.github/docs/session/command-queue.json` exists with `status: "PENDING"`. If
...
2. Updates `session-state.json`: adds the completed agent to `completed_agents`,
   ...
   agent.
3. Updates `.github/docs/session/pipeline-progress.json` for the web UI. Schema:
...
4. **Commits session-state.json to git** per ORC-47:
   `git add .github/docs/session/session-state.json .github/docs/session/pipeline-progress.json && git commit -m "checkpoint: [Agent Name] complete"`.
...
`.github/docs/contracts/`). The validation process:
...
`.github/docs/contracts/human-escalation-protocol.md`) sets the global status to
...
1. Run `node .github/scripts/github-state-snapshot.js` to capture the current
   ...
   `.github/docs/session/github-state-snapshot.json`.
...
1. Read `.github/docs/decisions.md` — extract all items with status `OPEN` and
   priority `HIGH`
2. Scan `.github/docs/decisions/` — for each `.md` file, read the category
...
5. **Decision injection (mandatory if `.github/docs/decisions.md` exists):**
   - Load all items with status `DECIDED` from `.github/docs/decisions.md`
     (index + uncategorized)
   - Scan `.github/docs/decisions/` — from each ACTIVE or PARTIAL category file,
...
7. **Lessons learned injection (mandatory if
   `.github/docs/retrospectives/lessons-learned.md` exists):**
...
3. Check: **secret scan PASSED**
   (`.github/docs/security/sprint-[SP-N]-secret-scan.md` present and status
   CLEAN)?
...
1. Receive Sprint Retrospective report
   (`.github/docs/retrospectives/sprint-[SP-N]-retrospective.md`)
...
`.github/docs/decisions/[file]` and edit the header block:

- Change `Status: DEFERRED` → `Status: ACTIVE`
- Change `Applicable: NO` → `Applicable: YES`
- Remove the `> Deferred-Reason:` line entirely

**Step 2 — Update the index file:** Open `.github/docs/decisions.md` and locate
...
item to `.github/docs/decisions.md` documenting the exception:
...
2. Create `.github/docs/session/session-state.json` with:
   ...
   ```
3. Create/update `.github/docs/session/pipeline-progress.json` to reflect the
