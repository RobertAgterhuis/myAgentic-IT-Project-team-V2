...
Must not finalize retention/deletion automation until approved entity-level
classification matrix is present.

### Violation Action

Mark `GUARDRAIL_VIOLATION: G-DATA-901`, block handoff of affected stories,
...
Pre-merge checklist validates classification artifact reference in policy/code
PRs.
...
Must always include schema version updates and backward-compatibility tests when
modifying persisted entity contracts.
...
Requires nightly integrity workflow validating schema conformance, markdown
parseability, and cross-file consistency.
...
Must not introduce new persistent data domain without assigning owner and
stewardship role.
...
Must map each dashboard KPI to catalog definition including formula, source
files, and owner.
