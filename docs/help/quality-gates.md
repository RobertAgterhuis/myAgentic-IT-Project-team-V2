...
the next phase can begin. This page explains what these gates check, how they
...
After every phase completes (all agents in that phase have handed off), the
Orchestrator automatically triggers a two-step validation:
...
This gate runs **4 times** during a full creation cycle (once per Phase 1–4) and
**once per sprint** during Phase 5 implementation.

> **Rule ORC-01:** The next phase NEVER starts before the current phase is fully
> completed AND validated by both the Critic Agent and the Risk Agent.

---

## What the Critic Agent Checks

The Critic Agent is the **quality guardian**. It does not produce analyses or
make recommendations — it only validates. It checks five things:

### 1. Contract Compliance

Every agent produces deliverables that must match an output contract. The Critic
checks:
...
The Critic loads `BusinessDocs/decisions.md` and verifies no recommendation
...
The Risk Agent performs an **independent risk assessment** on the same phase
output. It runs in parallel with (or immediately after) the Critic. Its focus is
broader:

### 1. Strategic Alignment

Are the phase outputs consistent with the business goals from Phase 1? For
example:
...
Like the Critic, the Risk Agent loads `BusinessDocs/decisions.md`. Any
...
If the same phase fails Critic/Risk validation **three times**, the Orchestrator
escalates to you with three options:
...
If the Critic returns PASSED with **zero findings** despite the phase having
open `UNCERTAIN:` or `INSUFFICIENT_DATA:` items, the Orchestrator flags this as
`CRITIC_REVIEW_SUSPICIOUS` and requires the Critic to justify why each open item
was not referenced.
...
During implementation, a lighter version of Critic + Risk runs **per sprint**.
The focus shifts to:
...
During a **Scope Change**, the Critic checks an additional requirement: every
re-analysis agent must include a `## Scope Change Impact` section as the first
section, stating which previous findings are confirmed, superseded, or net-new.
...
After Critic + Risk validation, the results are stored in the phase output
directory and referenced in `session-state.json`. The Orchestrator Log (in the
conversation) will also show whether the phase was approved, needs revision, or
is blocked.
