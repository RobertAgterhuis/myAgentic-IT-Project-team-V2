...
> Version: 1.0 | Defines the mandatory output structure for the Test Agent
> (Agent 21)
...
Ensures every implemented story is validated through unit tests, integration
tests, and acceptance criteria verification before it can be approved for merge.
The Test Agent produces a per-story test report that gates the PR/Review Agent's
work.
...
> **PARTIALLY_MET threshold:** `PARTIALLY_MET` requires the Test Agent to
> document which specific aspects are met and which are not. A story with any
> `PARTIALLY_MET` criterion cannot receive verdict `APPROVED` — it must be
> `FAILED` with remediation guidance.
...
> **Note:** Test Agent uses `APPROVED` / `FAILED`. `REJECTED` is deprecated —
> map any legacy `REJECTED` values to `FAILED`.
