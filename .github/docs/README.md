...
solutions — or auditing existing ones. The team consists of 38 specialized AI
agents that work together sequentially, each with a fixed role, dedicated skill
file, and output contract.

> **Key figures:** a complete Phase 1–4 cycle that takes 7–10 weeks manually
> costs only **5–10 working days** in calendar time with this agentic team —
> with just **7–12 hours of active attention** from you spread across that
> period. The bottleneck is your review speed, not the AI. Phase 5 sprints take
> **2–5 working days** per sprint instead of 1–2 weeks.
...
- A project brief or product idea (CREATE mode), or a codebase to be analyzed
  (AUDIT mode)
...
Replace `MyProject` with the name of your software. The Onboarding Agent will
ask you a series of intake questions. Answer them as completely as possible —
the quality of the entire output depends on it.

> Already have an existing codebase to analyze? Use `AUDIT MyProject` instead.
> See [Which command should I use?](#which-command-should-i-use) for all
> options.

**3. Track progress** via the generated files in `.github/docs/` and the GitHub
Kanban project that is automatically created.

Want a partial or targeted run? See
[Which command should I use?](#which-command-should-i-use) for the available
options.
...
- **CREATE mode** — Design and build a complete new software solution from
  scratch, from business strategy to implementation.
- **AUDIT mode** — Perform a comprehensive analysis of an existing software
...
Each department report is fully self-contained and includes a mandatory
**"Blockers from other teams"** section, so every department can act directly
without having to read the full master report.

### Generated business documents

Parallel to the analysis output, the system generates and maintains living
business documents in `BusinessDocs/`:
...
> **How do questionnaires work?** When an agent cannot find information in the
> provided input or documentation, it marks it as `INSUFFICIENT_DATA:`. After
> each phase, the Questionnaire Agent converts these gaps into a client-friendly
> questionnaire. You fill in the answers, then run `REEVALUATE` or a new cycle —
> the system automatically picks up your answers and produces a more
> comprehensive analysis.

> **What do you do with the reports?**  
> The department reports are directly usable as input for departmental meetings.
> The master report contains the 12-month roadmap that is automatically
> converted into GitHub Issues for Phase 5. You don't need to prioritize
> yourself — the Synthesis Agent, Critic Agent, and Risk Agent have already done
> that.
...
> **Canva API:** The Canva Connect API for brand kit functionality is currently
> invite-only. Request access at
> [developer.canva.com](https://developer.canva.com). This is entirely optional
...
> **Partial cycles:** Both CREATE and AUDIT support running 1, 2, or 3
> disciplines independently — e.g. `CREATE TECH UX MyProject`. Disciplines are
> always executed in canonical order (BUSINESS → TECH → UX → MARKETING).
> Produces the department reports for the chosen disciplines. **Combining
...
> on CREATE vs AUDIT mode**, see [.github/docs/mode-guide.md](mode-guide.md).
...
Type the desired command in the Copilot Chat in VS Code. The Onboarding Agent
starts automatically and asks intake questions about:

- **CREATE mode:** Product idea, target audience, value proposition, business
  model, budget, timeline
- **AUDIT mode:** Target audience and market, technical stack and
  infrastructure, available data
- **Both modes:** GitHub project name (for automatically creating the Kanban
  board), Canva API token (optional, for brand assets)

Answers are saved in `.github/docs/session/session-state.json`. The Orchestrator
uses this state throughout the entire cycle.

> Make sure to answer the intake questions completely — the quality of the
> entire output depends on the onboarding input. The more context you provide,
> the more concrete the results.

---

### Step 2 — Follow the phase sequence

The Orchestrator directs each agent in the mandatory sequence. You don't need to
route anything yourself. Each agent:

- Reads the output of the previous phase as mandatory input
- Produces its deliverables as Markdown + JSON files in
  `.github/docs/phase-[N]/`
- Closes with a **Handoff Checklist** — the next agent only starts once
  everything is checked off

After each phase, the **Critic Agent** and **Risk Agent** validate the output.
When uncertain about findings you will see:
...
Both are escalated to you via `.github/docs/decisions.md` or the Copilot Chat
...
After approval of the final report, the **GitHub Integration Agent** publishes
all stories as Issues in your GitHub project. Then Phase 5 starts:
...
artifacts can be found in `.github/docs/retrospectives/`.
...
Starts an isolated full cycle (Phases 1–5) for a single new feature. Output goes
to `Workitems/[FEATURENAME]/`, completely separate from the main backlog.

```text
REEVALUATE [scope]
```

Reassesses a previously completed component after a change (e.g.
`REEVALUATE TECH` after a major design revision or refactor). Generates a
Re-evaluation Report and injects impacts into the ongoing Sprint Gate.

```text
SCOPE CHANGE [DIMENSION]: [description]
```

Use when the **fundamental premise** of the project has changed — e.g. a
business model pivot, a core architecture switch, a new target audience
selection. `DIMENSION` is one of `BUSINESS | TECH | UX | MARKETING | ALL`. The
...
`.github/docs/synthesis/scope-change-[N].md`. This is distinct from
...
> `.github/docs/decisions.md` is your direct communication channel — you fill
> this in. All other files in `.github/docs/` are generated and overwritten by
...
Each decision or question gets a unique ID (`DEC-NNN`), a scope (e.g. `Phase 2`,
`SP-3`, `All sprints`), and a priority.
...
**Contracts** (`.github/docs/contracts/`) define what each agent is required to
...
**Guardrails** (`.github/docs/guardrails/00–09`) are testable, binding decision
...
The agent is missing required input. Check `.github/docs/decisions.md` for open
...
No. It means the agent did not have enough data for that specific finding, but
continued with what was available. If you supply the missing data later (e.g.
analytics data, usability test results), you can run `REEVALUATE [scope]`.

**An agent produces generic recommendations without source references — what
now?**  
...
`.github/docs/decisions.md` and restart the relevant agent.
...
scan + tooling) without asking new intake questions. The intake answers remain
intact.

**Can I roll back a previous sprint?**  
Sprint files are immutable. You can run a `HOTFIX` to fix a bug, or manually
move a story back in GitHub and wait for the next Sprint Gate. The Retrospective
Agent documents this as a LESSON_CANDIDATE.

**The GitHub Integration Agent cannot create a project — what is wrong?**  
Check that `GITHUB_PROJECT_NAME` is filled in
`.github/docs/session/session-state.json` and that your GitHub token has write
...
The bottleneck is not AI processing time, but your review speed between phases.
If you are available quickly for `UNCERTAIN:` escalations, you can easily reach
...
(~500k+ lines), expect the upper end of the above ranges.
