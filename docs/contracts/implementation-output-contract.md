...
> **SCOPE:** This contract applies exclusively to stories with `story_type`
> **`CODE`** or **`INFRA`**. Stories of type `DESIGN`, `CONTENT`, or `ANALYSIS`
> are **NOT** processed by the Implementation Agent pipeline. Upon receiving a
> story with a different type: `ROUTING_ERROR` → escalate to Orchestrator.
...
| Guardrails (all phases) | `docs/guardrails/00-09`                                               | YES      |
...
**HALT:** If any of the above mandatory inputs is missing, if `story_type` is
not `CODE` or `INFRA`, if `sprint_status` is not `IN_PROGRESS`, or if the
Blocker status is `EXTERN: [open]` → do NOT start implementation. Escalate to
Orchestrator.
...
**PROHIBITION:** Do not change code outside the story scope without an explicit
`SCOPE_EXTENSION: [reason]` notification.
...
**PROHIBITION:** A story is NOT complete if existing tests break due to the
implementation.  
**PROHIBITION:** A story is NOT complete if there is no test for every
...
**PROHIBITION:** A story with an unresolved VIOLATION may NOT proceed to Test
Agent.
...
**PROHIBITION:** `new_critical_findings` may NEVER be filled without escalation
to Orchestrator.
...
The Sprint Completion Report JSON (see "OUTPUT PER SPRINT" section above) serves
as the JSON export for this contract.
