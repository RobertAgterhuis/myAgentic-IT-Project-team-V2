...
- Framework defines: scoring aggregation, finding prioritization, backlog
  conversion
...
- **All 5 ACs completed:** experiment framework, A/B infra, baseline, workflow,
  guardrails
- Client-side A/B test: `headline-v1` (50/50 split)
- Control: "Design it right. Build it fast." vs Variant A: "AI-Powered
  Phase-Based SDLC for Product Teams"
- Matomo Custom Dimension 1 tracks variant assignment
- `trackPageView` deferred from head to experiment script (ensures dimension set
  before tracking)
- Statistical guardrails: min 100 / target 385 per variant, p < 0.05,
  anti-peeking rule
...
(landing experiment) was the largest implementation item (5 ACs + code + 25
tests).
