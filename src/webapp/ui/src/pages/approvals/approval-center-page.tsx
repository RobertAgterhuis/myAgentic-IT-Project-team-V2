...
import { CodeEditorPane, DiffReviewPane } from '@/components/cockpit/monaco-host-panels';
...
import type {
  MonacoGovernanceAnnotation,
  MonacoGovernanceOverlay,
} from '@/components/cockpit/monaco-host-panels';
import {
  ShieldCheck,
  CheckCircle,
  XCircle,
...
function ApprovalDecisionPanel({
  ...
  const rejectReasonModelLocator = useMemo<MonacoModelLocator>(
    () => ({
      namespace: 'workspace',
      objectId: `approval-${approvalId}-reject-reason`,
      path: `approvals/${approvalId}/reject-reason.md`,
      language: 'markdown',
    }),
    [approvalId]
  );
  ...
  const riskAnnotations: MonacoGovernanceAnnotation[] =
      ...
      ? [
          {
            ...
            lineNumber: 1,
            category: 'risk',
            label: `Risk level: ${(approval as { risk_level?: string }).risk_level}`,
            detail: 'Risk classification attached to approval context.',
          },
        ]
      : [];

  const governanceOverlay: MonacoGovernanceOverlay = {
    mode: 'enforced',
    policyId: 'approval-governance-policy',
    readOnlyByPolicy: !isPending,
    policySummary: 'Only pending approvals can be edited; decided approvals are read-only.',
    annotations: [
      {
        ...
        lineNumber: 1,
        category: 'decision',
        label: `Decision status: ${approval.status}`,
        detail: `Requested by ${approval.requested_by}`,
      },
      {
        id: `${approval.id}-gate`,
        side: 'modified',
        lineNumber: 1,
        category: 'gate',
        label: `Gate context: ${approval.gate_id}`,
        detail: `Stage ${approval.stage}`,
      },
      {
        id: `${approval.id}-provenance`,
        side: 'modified',
        lineNumber: 1,
        category: 'provenance',
        label: `Provenance: approval ${approval.id}`,
        detail: `Entity ${approval.entity_id}`,
      },
      ...riskAnnotations,
    ],
  };
    ...
    <div className="flex flex-col gap-4 p-4" data-testid="approval-decision-panel">
      ...
      {approval.comparison && (
        <DiffReviewPane
          ...
          modifiedLabel="After"
          originalContent={approval.comparison.before}
          modifiedContent={approval.comparison.after}
          originalModelLocator={{
            namespace: 'artifact',
            objectId: `${approval.id}-comparison-before`,
            path: `approvals/${approval.id}/comparison-before.md`,
            language: 'markdown',
          }}
          modifiedModelLocator={{
            namespace: 'artifact',
            objectId: `${approval.id}-comparison-after`,
            path: `approvals/${approval.id}/comparison-after.md`,
            language: 'markdown',
          }}
      ...
      <div className="space-y-3">
        <CodeEditorPane
          title="Rejection rationale (required to reject)"
          value={rejectReason}
          onChange={setRejectReason}
          modelLocator={rejectReasonModelLocator}
          governanceOverlay={governanceOverlay}
        />
