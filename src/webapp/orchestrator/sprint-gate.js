...
const DECISIONS_PATH = '.github/docs/decisions.md';
const LESSONS_LEARNED_PATH = '.github/docs/retrospectives/lessons-learned.md';
const VELOCITY_LOG_PATH = '.github/docs/retrospectives/velocity-log.json';
const BLOCKER_MATRIX_PATH = '.github/docs/synthesis/cross-team-blocker-matrix.md';
const REEVALUATE_TRIGGER_PATH = '.github/docs/session/reevaluate-trigger.json';
 ...
 */
function loadReevaluateTrigger(store, triggerPath) {
 ...
 */
function checkDefinitionOfReady(stories) {
  const issues = [];
 ...
 */
function parseLessonsLearned(content) {
  const lessons = [];
  const lines = content.split('\n');
 ...
 */
function parseVelocityLog(jsonContent, window) {
 ...
 */
function checkVelocityCapacity(store, plannedItems, opts = {}) {
  const velocityPath = opts.velocityPath || VELOCITY_LOG_PATH;
  const threshold = opts.capacityThreshold || CAPACITY_THRESHOLD;
  const issues = [];
  ...
  const { trailingAverage, sprintCount } = parseVelocityLog(content, opts.window);
 ...
 */
function parseBlockerMatrix(content) {
  const blockers = [];
  const lines = content.split('\n');
  let inTable = false;
 ...
 */
function checkBlockers(store, blockerPath) {
...
function runSprintGate(store, options) {
  ...
  if (!sprintId) {
    return {
      ...
      steps: {},
      summary: { sprintId: null, timestamp: new Date().toISOString() },
    };
  }
