...
- **Backend CRUD**: POST (create), PUT (update), PATCH (archive) — SP-9.1/9.2/9.3
- **Frontend Modals**: Create, Edit, Delete with API integration — SP-9.8/9.4/9.5
...
- **Expected Result**: Modal appears with title "New Milestone", form is empty, all fields visible
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Expected Result**: Error message displays "Name is required", form not submitted
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Expected Result**: Should accept (0% is valid), milestone created with progress 0
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 

### Test Case 1.4: Successful Creation
- **Setup**: Create modal is open
- **Action**: Fill Name="FEATURE-99 Testing", Status="in progress", Progress=50%, Completion="2026-04-15", click Submit
- **Expected Result**: 
  - Modal closes
...
- **Expected Result**: Error message "Milestone name already exists", form not submitted
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Action**: Change Status="complete", Progress=100%, Completion="2026-03-09", Submit
- **Expected Result**: 
...
- **Setup**: Milestones "TASK-01" and "TASK-02" exist; Edit modal open for "TASK-02"
...
- **Action**: Fix the error and resubmit (e.g., change duplicate name to unique one)
- **Expected Result**: 
...
- **Setup**: Table has milestones: "FEATURE-01 Auth", "FEATURE-02 Dashboard", "TESTING Phase 3"
...
- **Expected Result**: "FEATURE-01 Auth" and "FEATURE-02 Dashboard" displayed, case-insensitive
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Setup**: Table has milestones with various statuses (not started, in progress, complete, blocked)
...
- **Setup**: Table has milestones with completion dates: 2026-02-15, 2026-03-01, 2026-03-15, 2026-04-01
...
- **Expected Result**: Only milestones containing "FEATURE" AND status="in progress" shown
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Expected Result**: Only milestones with completion in March AND progress >= 50% shown
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 

### Test Case 8.3: All Four Filters Active
- **Setup**: Table loaded with diverse data
- **Action**: Activate search="API", status="complete", date from=2026-03-01, progress=75%
...
- **Action**: Submit form
- **Expected Result**: Error toast displays "Failed to create milestone" or similar
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Expected Result**: 
  - API validation catches conflict or overwrites with latest (per business rule)
...
- **Expected Result**: Should accept (no validation restriction on special chars per spec)
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
...
- **Expected Result**: Created milestone still present in table with all data intact
- **Status**: [ ] PASS / [ ] FAIL
- **Notes**: 
