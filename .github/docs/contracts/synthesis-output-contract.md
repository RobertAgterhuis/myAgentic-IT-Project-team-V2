...
> Version: 1.0 | Defines the mandatory output structure for the Synthesis Agent
> (Agent 17)
...
Ensures the Synthesis Agent produces the complete set of six documents that
consolidate all phase outputs into actionable executive reports,
department-specific reports, and a cross-team blocker matrix. These documents
represent the final deliverables of the design/strategy phases and gate entry
into Phase 5 (Implementation).

---

## OUTPUT FILES

**Location:** `.github/docs/synthesis/` **Format:** Markdown **Required files
(6):**
...
7. **Open Items** — All unresolved `UNCERTAIN:` and `INSUFFICIENT_DATA:` items
   with owner
...
4. **Sprint Plan Items** — Stories/tasks derived from this discipline, mapped to
   sprint IDs
5. **Blockers from Other Teams** — Explicit section listing cross-team
   dependencies (even if none exist, state "No blockers identified")
...
- [ ] Cross-team blocker matrix classifies every dependency as BLOCKING or
      ADVISORY
- [ ] No open `UNCERTAIN:` or `INSUFFICIENT_DATA:` items without a corresponding
      entry in Open Items
...
> No standalone JSON export. Sprint plan items are consumed from department
> reports' Section 4. The GitHub Integration Agent parses structured Markdown
> tables directly.
