...
> Version: 1.0 | Defines the mandatory output structure for the Brand & Assets
> Agent (Agent 30)
...
Ensures the Brand & Assets Agent produces design tokens, brand guidelines, and a
brand assets report that form the visual identity foundation for the project.
These outputs feed into the Storybook Agent's component inventory and the
PR/Review Agent's brand compliance checks.
...
- **colors:** Primary, secondary, accent, background, text, error, warning,
  success
...
3. **Color System** — Color palette with hex/RGB values, usage guidelines,
   contrast ratios
4. **Typography** — Type scale, font pairings, usage contexts
5. **Imagery & Iconography** — Style guidelines, icon system, photography
   direction
...
3. **Accessibility Compliance** — Color contrast ratios verified (WCAG AA
   minimum)
...
- [ ] `design-tokens.json` exists and is valid JSON (or `SKIPPED_NO_TOKEN` is
      documented)
...
The `design-tokens.json` file serves as the JSON export for this contract.
...
All six keys MUST be present. Contents follow the structure defined in the
`design-tokens.json` section above.

---

## Refresh Protocol

When `BRAND_REFRESH_REQUIRED: YES` is set by the Reevaluate Agent, the Brand &
Assets Agent re-executes with the reevaluation delta as input, produces updated
tokens and guidelines, and increments the `version` field in
`design-tokens.json`.
...
3. Brand & Assets Agent produces updated `design-tokens.json` +
   `brand-guidelines.md`
...
6. Synthesis is BLOCKED until refresh completes (per BRAND_REFRESH_REQUIRED
   flag)
...
- `SKIPPED_NO_TOKEN` — Design tokens not applicable; brand-guidelines.md still
  required
