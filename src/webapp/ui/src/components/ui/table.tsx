...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-[26px] border border-border/70 bg-gradient-to-br from-card via-card to-info/6 shadow-sm backdrop-blur-sm"
    >
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        'border-b border-border/60 transition-colors hover:bg-background/70 data-[state=selected]:bg-info/8',
        className
      )}
      {...props}
    />
