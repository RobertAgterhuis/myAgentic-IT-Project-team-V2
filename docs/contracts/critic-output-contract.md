...
> Version: 1.0 | Defines the mandatory output structure for the Critic Agent
> (Agent 18)
...
Ensures every phase output undergoes rigorous quality, completeness, and
compliance review before handoff. The Critic Agent validates all agent outputs
within a phase against their respective contracts, anti-hallucination protocol,
and guardrails, producing an itemized validation report with a clear APPROVED or
FAILED verdict.
...
**Location:** `docs/[phase]/critic-risk-validation.md` (critic section)
**Format:** Markdown
...
- **Contract Compliance:** Does the output satisfy all MANDATORY SECTIONS of the
  agent's output contract? Itemized pass/fail per section.
- **Anti-Hallucination Compliance:** Are all claims sourced? Are `UNCERTAIN:`
  and `INSUFFICIENT_DATA:` prefixes used correctly? No fabricated metrics?
...
- [ ] Every FAILED verdict includes at least one itemized finding with source
      reference
...
> No standalone JSON export for this contract. The Critic Agent's output is
> Markdown-only; findings are consumed by the Orchestrator from the structured
> Markdown sections.
